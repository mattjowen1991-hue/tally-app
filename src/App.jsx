...
function AppContent() {
  ...
  const handleTogglePaid = (id) => { setBills(bills.map((b) => { if (b.id !== id) return b; if (b.missed) return { ...b, paid: true, missed: false }; return { ...b, paid: !b.paid, missed: false }; })); haptic.medium(); };
  const handleToggleMissed = (id) => { setBills(bills.map((b) => (b.id !== id ? b : { ...b, missed: !b.missed, paid: false }))); haptic.warning(); };
  const handleTogglePaused = (id) => { setBills(bills.map((b) => (b.id !== id ? b : { ...b, paused: !b.paused }))); haptic.light(); };
  ...
  const handleArchiveDebt = (id) => { setDebts(debts.map((d) => d.id !== id ? d : { ...d, archived: true, archivedAt: new Date().toISOString() })); haptic.success(); toast('Debt archived', 'success'); };
  const handleUnarchiveDebt = (id) => { setDebts(debts.map((d) => d.id !== id ? d : { ...d, archived: false, archivedAt: null })); haptic.medium(); toast('Debt restored', 'info'); };
  ...
  const handleMakePayment = (debtId) => { const amount = parseFloat(debtPaymentAmounts[debtId]); if (!amount || amount <= 0) return; const debt = debts.find(d => d.id === debtId); const newTotal = Math.max(0, (debt?.totalAmount || 0) - amount); setDebts(debts.map((d) => d.id !== debtId ? d : { ...d, totalAmount: newTotal, payments: [...(d.payments || []), { date: new Date().toISOString(), amount, type: 'manual' }], ...(newTotal === 0 ? { archived: true, archivedAt: new Date().toISOString() } : {}) })); setDebtPaymentAmounts({ ...debtPaymentAmounts, [debtId]: '' }); haptic.success(); toast(newTotal === 0 ? '🎉 Debt paid off!' : `£${amount.toFixed(2)} payment made`, 'success'); };
  ...
  const handleArchiveSavings = (id) => { setSavings(savings.map((s) => s.id !== id ? s : { ...s, archived: true, archivedAt: new Date().toISOString() })); haptic.success(); toast('Goal archived', 'success'); };
  const handleUnarchiveSavings = (id) => { setSavings(savings.map((s) => s.id !== id ? s : { ...s, archived: false, archivedAt: null })); haptic.medium(); toast('Goal restored', 'info'); };
  ...
  const doSavingsTransaction = (goalId, amount, type) => { const goal = savings.find(s => s.id === goalId); const newAmount = Math.max(0, (goal?.currentAmount || 0) + amount); const hitTarget = goal?.targetAmount > 0 && newAmount >= goal.targetAmount && (goal.currentAmount || 0) < goal.targetAmount; setSavings(savings.map((s) => s.id !== goalId ? s : { ...s, currentAmount: newAmount, transactions: [...(s.transactions || []), { date: new Date().toISOString(), amount, type }], ...(hitTarget ? { archived: true, archivedAt: new Date().toISOString() } : {}) })); setSavingsTransactionAmounts({ ...savingsTransactionAmounts, [goalId]: '' }); if (hitTarget) { haptic.success(); toast('🎉 Goal reached!', 'success'); } else { haptic.medium(); toast(type === 'deposit' ? `£${Math.abs(amount).toFixed(2)} deposited` : `£${Math.abs(amount).toFixed(2)} withdrawn`, type === 'deposit' ? 'success' : 'warning'); } };
